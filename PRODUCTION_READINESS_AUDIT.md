...
| Duplicates | Review is opt-in and never deletes automatically. **Resolved:** the 20k/quadratic path was replaced by a durable, resumable, all-library job (migration `00018`, `internal/duplicates`) — LSH candidate buckets (`bands()`) + union-find, coverage/version/completion tracked via `duplicate_runs`. `/api/duplicates` serves the persisted run, not a request-time computation. | — (was: newest-20k pairwise quadratic) | Done. |
