export type MemoryAsset = { taken_at?: string; taken_day?: string };

/**
 * A memories rail entry: one year's worth of "on this day", with the newest
 * asset in that year standing as its cover.
 */
export type MemoryGroup<T extends MemoryAsset = MemoryAsset> = {
 ...
 */
export function memoryGroups<T extends MemoryAsset>(assets: T[], now: Date = new Date()): MemoryGroup<T>[] {
  const thisYear = now.getFullYear();
  const byYear = new Map<number, { cover: T; count: number }>();

  for (const asset of assets) {
    if (!asset.taken_at) continue;
    const year = Number((asset.taken_day ?? asset.taken_at).slice(0, 4));
