...
- `feat/gestures-and-substring-search` (2026-08-15) — **Search only matched the start of a name, and the grid had no gestures.**
  ...
  - **Verified:** `make check`, `make check-gen`, `tsc --noEmit`, `expo lint`, 211 mobile tests, `check-tokens`, plus simulator screenshots of the search screen. **Not verified: the gestures themselves** — multi-touch cannot be driven through `simctl`, so drag-paint, pinch and swipe-to-dismiss need a hand on a device.
