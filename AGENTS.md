...
- **Mobile onboarding redesign (2026-09-13, `codex/micro-interactions`).** The existing setup gate
  remains four routes (welcome → server → pair → permissions), but they now render through one
  responsive 520pt frame with shared fields, cards, notices, and animated buttons. The setup layout
  owns a persistent full-width progress rail so it animates between routes rather than restarting per
...
- `codex/micro-interactions` (2026-09-13, onboarding) — **Mobile setup is now a modern guided flow
  ...
  responsive readable width, consistent cards/fields/notices/buttons, and short entrance motion;
  the layout owns an accessible, label-free rounded progress rail. Welcome copy is
  shorter, server errors are contained, pairing clearly separates QR from manual code, and the final
  permission step promotes the ready state without blocking a deliberate skip. Reduce Motion turns
  off page travel and progress interpolation. This was statically verified, not visually certified
  on a physical device; ROADMAP.md's mobile release gate remains open.
...
- `feat/gestures-and-substring-search` (2026-08-15) — **Search only matched the start of a name, and the grid had no gestures.**
  ...
  - **Seven places wrote `assets_fts` by hand.** Importer, OCR worker, metadata editor, trash purge, and both external-library paths. Adding a second index to seven hand-written statements is how one gets missed, and a missed one fails silently — the asset just stops appearing in some searches. `internal/fts` owns both tables now; a third tokenizer is one line there.
  - **The mobile search screen was the app's only `@expo/ui/swift-ui` importer**, which is iOS-only while Kuraki ships an Android APK. Android's `TextFieldView` emits `onValueChange` where the swift-ui wrapper listens for `onTextChange`, so **every keystroke was swallowed and no search ever ran**; `PickerView` is not registered on Android at all. Neither `tsc` nor `expo lint` can see this — the types are valid on both platforms — so `platform-parity.test.ts` guards it mechanically. The field is now the navigation bar's own search bar (which iOS 26 renders in the tab bar, since the tab is `role="search"`) and the filters are `@expo/ui/community/segmented-control`, which ships a real `.android.tsx`.
  - **`headerShown: false` takes `headerSearchBarOptions` down with it.** Verified on device: hiding the now-empty header also removed the search field from the tab bar, leaving no way to enter a query. The header stays, with an empty title.
  - **Gesture decision logic lives in `src/lib`, not in the components.** `drag-select`, `grid-zoom`, `viewer-gestures` — because `mobile/` has no React Native render harness, so anything left inside a component is checked by eye and nowhere else. 20 unit tests across the three.
  - **Drag-select activates horizontally-first on purpose.** A vertical drag must keep scrolling, and both gestures share one scroll view. The cost, stated: dragging straight down a column scrolls rather than paints. The drag's direction is fixed by the tile it starts on, and its running set lives in the gesture — the `selectedIds` prop arrives a render too late to drag with.
  - **The viewer's `GestureHandlerRootView` had to be restored.** It was removed when the last gesture in there went away; a Modal is its own native window the app-level root does not reach, so without it the new gestures are silently dead on Android.
  - **Two lint rules were load-bearing.** `react-hooks/set-state-in-effect` caught a real synchronous setState in the search effect; `react-hooks/refs` caught `useRef(new Animated.Value()).current` (fixed — `useMemo` is the codebase's own idiom). Its complaint about handing callbacks to `.onUpdate()` is a false positive for JS-thread gestures and is disabled in two places **with the reasoning written down**.
  - **Follow-up in the same branch:** drag-select is now *two* raced pans — sideways claims immediately, a ~220ms hold claims any direction. The first version only had the sideways rule, which meant a vertical drag could only ever scroll and selecting a column was one tap per tile. A plain vertical drag with no hold still scrolls.
  - **The offline mirror's writes had no `.catch`.** A failed SQLite write became an unhandled rejection, which expo renders as a red banner over the UI; the request it came from had already succeeded, so there was nothing for the user to act on.
  - **`react-hooks/refs` is genuinely at odds with RNGH's builder API.** `.onUpdate(fn)` is a call made during render, so a `fn` that touches a ref trips the rule even though nothing invokes it until a finger does. Suppressed in two places with the alternatives recorded: holding the state in a `useState` initialiser was tried and the compiler rejects it *harder* ("This value cannot be modified"), and Reanimated shared values would make these the codebase's only worklets for no behavioural gain. The rule did catch one real bug — a ref read during render for an `Animated.Value` — which was fixed, not suppressed.
  - **Verified:** `make check`, `make check-gen`, `tsc --noEmit`, `expo lint`, 211 mobile tests, `check-tokens`, plus simulator screenshots of the search screen. **Not verified: the gestures themselves** — `simctl` has no touch injection and `idb` is not installed, so drag-paint, pinch and swipe-to-dismiss need a hand on a device.

- `fix/filter-panel-and-site-longform` (2026-08-15) — **The filter panel rendered below the entire photo grid.**
  - **Opening Filters appeared to do nothing.** The panels were markup placed after `</LibraryView>`, so they landed under every photo — on a real library you had to scroll past the whole grid to find the controls you had just asked for. `LibraryView` now exposes a `subheader` slot directly under `PageHeader`, which is where a page's own controls belong.
  - **The header's jump-to-date was a duplicate of the panel's From.** Two controls writing one field, one of them a bare `dd/mm/yyyy` input sitting in the chrome. Removed; From/To in the panel is the single place dates are set.
  - **Eleven controls on one wrap row read as an undifferentiated strip.** The panel is a `repeat(auto-fill, minmax(190px, 1fr))` grid now, so each field gets a column and wraps predictably, with label and control locked together.
  - **The site's `/download` had no vertical rhythm and no measure.** The landing page gets its rhythm from gaps between its own sections; a prose page has none of that structure, so headings sat directly on the paragraph above and lines ran the full 1090px — about twice a readable measure. New `.longform` class: 72ch, space above headings rather than below (that is what binds a heading to the text it introduces), and the download table exempted since it is facts, not prose.
  - **Two real copy/UX defects on the site.** Astro collapsed the newline between "see" and a link, rendering **"seethe media support matrix"** — fixed with an explicit `{' '}`. And docs code blocks clipped mid-token (`…kuraki:l`), so a reader could not see the `docker run` command they were meant to copy; long commands wrap now.
  - **Verified:** `make check`, `npm run check`, `check-contrast.py`, 93 Playwright, site build + token gate + docs-links, and the panel measured as sitting above the grid (panel top 99, grid top 291).

- `fix/web-full-width` (2026-08-15) — **The timeline was capped at 1440px, and I had previously mis-diagnosed it as correct.**
  - **`.content` was `width: min(1440px, 100%)`, which capped EVERY page**, the timeline included. On a 1920 screen the grid stopped ~280px short and the app looked like it had failed to fill the window. Photo surfaces are now full width; surfaces that are READ set their own measure (`.settings-shell` is bounded to 1040px so the rail and panel stay one block rather than the 180px rail being marooned across a wide field).
  - **This corrects an earlier wrong call, recorded because the reasoning is the trap.** During the spacing pass I saw the grid filling only part of a 1920 viewport and concluded `auto-fill` was behaving correctly against a sparse fixture library — plausible, and wrong. A screenshot of a REAL library with 21 photos showed it still stopping short. The tell was there all along and I did not read it: **the page header ended at the same x as the grid**, so it was never the grid, it was the column. Fixture data can make a layout bug look like a data artefact.
  - **The first version of the regression test was worthless, and only checking made that visible.** It asserted the grid fills >90% of available width at a **1600px** viewport — but the cap being guarded against is 1440px, which at 1600 still clears 90%. It passed with the bug fully reinstated. Widened to 1920, it now fails with `grid is 1392px of 1700px available — is .content capped again?`. **A guard against a specific value must run at a size where that value is detectable.**
  - **Verified:** `make check`, `npm run check`, `check-contrast.py`, 93 Playwright, and the assertion confirmed to fail against the reinstated cap.

- `fix/external-library-authz` (2026-08-15) — **A real privilege escalation: any signed-in account could index every other owner's photos into its own library.**
  - **The chain.** `POST /api/external-libraries` checked only for *a* session, not a role. `root_path` was taken from the request, `filepath.Abs`'d, and handed to `external.Scan`, which walks it recursively and inserts **every media file it finds as an asset owned by the caller**. So a non-admin could post `{"root_path": "<data>/originals"}` — the directory holding every owner's files — and claim the lot, then read and download them through the ordinary endpoints.
  - **Why the existing guard could not see it.** `ownerscope_guard_test.go` fails the build on unscoped asset SQL, and this SQL is *correctly* owner-scoped — it writes `owner_id = caller`. The scoping was never the hole; the **files** were attacker-chosen. A static check on query text cannot catch a correctly-written query pointed at the wrong data.
  - **Admin-only is necessary but not sufficient.** AGENTS.md states "an admin manages accounts, **not** photos — there is no path from one account to another's library", so gating on `requireOwner` alone would have handed admins exactly that path. The fix is both: the three write routes take `requireOwner`, **and** a root that resolves inside the storage base is refused. Containment rather than an allowlist, because indexing arbitrary mounted folders is the whole point of the feature. `filepath.EvalSymlinks` on both sides, so a symlink pointing back into the data directory cannot walk around it.
  - **Listing stays open** — `GET` only ever returns the caller's own rows.
  - **Both tests were confirmed to catch the original bug**, not merely to pass: reverting the route guard yields `non-admin create = 201, want 403`, and disabling the containment check yields `201, want 400`.
  - **Verified:** `make check`, `make check-gen`, 93 Playwright. Found by reading the endpoint surface after the dependency audit came back clean — the dependency scanners had nothing to say about this.

- `fix/security-dependency-updates` (2026-08-15) — **Six Go vulnerabilities, four npm ones, and the one missing security header.**
  - **`golang.org/x/image` v0.44.0 → v0.45.0 is the one that mattered most.** It is the pure-Go decoder for *untrusted uploaded images* — the single most attacker-reachable dependency in the project. Five more were Go stdlib issues (`net/http`, `crypto/tls`, `net/url`, `encoding/asn1`) fixed in 1.26.6 against a 1.26.5 toolchain; the `go` directive is now `1.26.6`, which is also what CI resolves through `go-version-file`. **govulncheck now reports zero.**
  - **Two "fixes" were refused, and the reasons matter more than the fixes.** `npm audit fix --force` on web would **downgrade @sveltejs/kit from 2.70.2 to 0.0.30** — npm's resolver producing a nonsense result — to patch a `cookie` advisory whose code path does not exist here at all: `adapter-static` ships a static SPA served by the Go binary, with no SvelteKit server at runtime. On mobile the forced fixes would downgrade **react-native to 0.72.17 and expo to 53**; the remaining advisories are Metro/prebuild toolchain (`image-size`, `xcode`, `uuid`), i.e. build-time DoS parsers, not the shipped app. All non-breaking fixes WERE applied on both.
  - **HSTS was the only missing header, and it is deliberately conditional.** The app already sets CSP-with-nonce, `nosniff`, `X-Frame-Options: DENY`, `Referrer-Policy`, `Permissions-Policy` and same-origin write protection. HSTS now rides on `KURAKI_SECURE_COOKIES`, the operator's existing "I am behind HTTPS" declaration. **Sending it unconditionally would be worse than omitting it:** a browser that sees HSTS once refuses plain HTTP for that host for the max-age, and the default deployment of a self-hosted photo server is `http://a-box-on-my-LAN:3000` — it would lock people out of their own library with no server-side undo. The test asserts both directions.
  - **Changing the middleware signature broke an existing test, which is the system working.** `securityHeaders` became `securityHeaders(bool)`, and `TestSecurityHeaders` called it directly.
  - **Verified:** govulncheck clean, `make check`, `make check-gen`, `check-docs-links`, `npm run check`, 16 Vitest, 93 Playwright, mobile `tsc --noEmit` + 188 Vitest.

- `feat/web-spacing-and-density` (2026-08-14) — **Measured the rhythm instead of eyeballing it, and found the shared chrome was off it everywhere.**
  - **The audit is the interesting part.** A script walked every route reading *computed* `gap`/`padding`/`margin` off every element and flagged anything not divisible by the host register's step (8 Kura / 4 Vault), plus how much of the fold each page's content actually fills. Eyeballing spacing does not find this; measuring does.
  - **Two fractional multipliers were the whole systemic problem.** `calc(var(--space-step) * 1.5)` renders **12px on a page whose scale has no 12, and 6px on a page whose scale has no 6** — it is off-rhythm in *both* registers simultaneously. It was in `PageHeader` and `EmptyState`, which appear on nearly every route, so those were the most-repeated off-scale numbers in the app. Integer multipliers only now, and a test asserts it per register. The settings rail's `padding: 7px 10px` was two more values on no scale at all.
  - **Sparse pages were wide, not empty.** Measured at 1440, settings rows ran the full ~1200px with content stopping after a third of it, so a label sat a long way from its control and pages filled 24–77% of the fold as a wide sparse field. The panel now has a **68ch reading measure**: label and control come back together, and the space to the right reads as margin rather than as something missing. `/settings` went 63% → 76% fill without adding anything.
  - **The photo surfaces are deliberately NOT capped.** A timeline should use every pixel it is given; only the surfaces that are *read* get a measure. The test asserts both halves so a future "consistency" pass cannot cap the grid.
  - **Researched rather than invented.** Justified layouts (Google Photos, Flickr, Immich's dynamic mode) use more screen width than square crops — but Kuraki's gapless square proof-sheet is a documented identity choice, so that is not the lever and the grid was left alone. The 8pt/4pt rhythm and the reading measure are the parts of the external guidance that actually apply here.
  - **Verified:** `make check`, `npm run check`, 16 Vitest, **93 Playwright** (5 new), `check-contrast.py`, both palette gates, and the audit re-run showing the shared chrome now on-scale.

- `fix/web-capture-time-utc` (2026-08-14) — **The viewer's date now agrees with the heading it sits under, and the open question is answered.**
  - **The answer was in the server, not in taste.** `taken_day` is a **string prefix of `taken_at`** (`httpapi/assets.go:315` — `row.TakenAt.String[:10]`), and `taken_at` is stored UTC. So the day a photo groups under IS its UTC date, `labelDate` already formats in UTC to match, and the viewer formatting the same instant in the browser's zone was the only outlier. A photo filed under "Jul 1" could read "Jun 30, 7:00 PM" in the panel directly beneath that heading.
  - **It is also the more accurate reading of the value.** EXIF's `DateTimeOriginal` carries no zone — it is the camera's wall-clock time — and the importer stores it via `taken.UTC()`, so for any photo with EXIF the stored instant IS the local time it was taken. Rendering in UTC hands it back unchanged, which is what every photo application shows.
  - **The residual limitation, stated rather than hidden:** no capture offset is stored anywhere, so a photo dated from file mtime (the fallback for media with no EXIF) is a true instant and reads in UTC rather than where it was taken. Fixing that means storing the offset at import — a schema change and a migration, not a formatting choice. Recorded beside the formatter.
  - **The test was wrong twice before it was right, and both mistakes are instructive.** First it used `23:30Z`, which in America/Los_Angeles is still the same day — it passed with and without the fix and proved nothing. Only an EARLY-UTC instant (`02:00Z`) crosses the date line westward. Then it asserted the US pattern (`'1,'`) while the environment resolves to en-GB (`"1 Jul 2025"`). It now asserts every token of the heading appears in the viewer's string: locale-agnostic, and it fails with the bug spelled out — *viewer shows "30 Jun 2025, 7:00 pm" under a heading of "1 Jul 2025"*.
  - **Verified:** `make check`, `npm run check`, 16 Vitest, 88 Playwright, and the new test confirmed to fail without the fix.

- `fix/web-ui-round-2` (2026-08-14) — **Dark mode and the narrow widths, checked for the first time.**
  - **What came back clean is the point.** A rendered contrast sweep — real computed colours against real backgrounds on every route, not just the palette file — found **nothing** below 4.5:1 in dark mode, and there is no horizontal overflow at 390/768/820/1024/1920. `check-contrast.py` and the single 820 seam are both earning their keep.
  - **The 1920 grid is correct, and worth not "fixing".** Tiles fill only part of the width because `auto-fill` keeps tile size consistent across days; `auto-fit` would stretch a two-photo day into two enormous tiles. Left alone deliberately.
  - **A numbered process rendered with no numbers.** Tailwind's preflight sets `list-style: none` on every list, which is right for the navigation and card grids that make up most lists here — and wrong for the phone-pairing `<ol>`, where the order is the information. Restored, scoped to that list. This is the one place in the app where numbered markers are earned rather than decorative.
  - **The 320px overflow took three attempts and two of them were wrong.** `min-width: 0` on the rail and `minmax(0, 1fr)` on its grid track were both real improvements but neither was the cause. The culprit was "Run integrity check" + "Scan for duplicates" side by side needing ~330px in a non-wrapping row. **The lesson is the diagnostic, not the fix:** an element inside a horizontal scroller legitimately has `getBoundingClientRect().right > innerWidth`, so a naive overflow probe reports the scroller's children and buries the real offender. Skip anything with a scrolling ancestor.
  - **And one probe measured nothing at all.** A direct check reported `doc == 320` — no overflow — because it never waited for `main#main` and was reading the sign-in page. It "disproved" a finding that was real. Always assert you are on the page you think you are.
  - **Also:** `/settings/users` was the last page still on sentence-case bold headings instead of `SectionHeading`; Activity's empty state was the only one without a mark; and the integrity readout rendered "36 checked· 24s ago" because Svelte collapses whitespace around an `{#if}` boundary — `{' · '}` restores it.
  - **Verified:** `make check`, `npm run check`, 15 Vitest, **88 Playwright** (4 new), `check-contrast.py`, both palette gates, and a re-run of the audit script that now reports **no findings** at any width or theme.
