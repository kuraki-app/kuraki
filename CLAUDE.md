...
**The web UI has two registers, and the seam is a rule.** `web/src/app.css` defines one palette driven by two sets of `--frame-*`/`--space-step` tokens: **Kura** (8px rhythm, large settled headings, soft cards — photo pages) and **Vault** (4px rhythm, micro-caps labels, tabular figures, hairline panels — operational pages). Both set type in one system sans: the split is rhythm, density and treatment, never typeface. `nav.ts` assigns the register per route and `+layout.svelte` writes it to `data-register` on `<main>`.
